...
public class Aplicacion {
	...
	public void ejecutarOpcion(int opcionSeleccionada)
	{
		...
		else if(opcionSeleccionada==2)
		{
			...
			{
				System.out.println("¿Cuál de las bebidas del menú desea agregar?");
				System.out.println("----------------------------------------------");
				ArrayList<Bebida> bebidas=restaurante.getBebidas();
				for (int i=1; i<=bebidas.size();i++)
				{
					System.out.println(i+". "+bebidas.get(i-1).getNombre()+"-$"+bebidas.get(i-1).getPrecio());
				}
				int bebidaAgregar=Integer.parseInt(scanner.nextLine());
				restaurante.getPedidoEnCurso().agregarProducto(bebidas.get(bebidaAgregar-1));
			}
			else
			{
				System.out.println("Opción desconocida");
